<x-app-layout>
    ...
    <div class="container mx-auto mt-8 p-5 rounded-xl bg-gradient-to-r from-sky-200 to-pink-200 shadow-md">
        ...
        @if (!$question['next'])
            <form action="{{ route('start_tembang_dolanan_game.check', [
                'tembangDolanan' => $tembangDolanan,
            ]) }}" method="POST" class="flex justify-center mt-12">
                @csrf
                <button type="submit"
                    class="flex gap-2 items-center bg-gradient-to-r text-xl from-pink-500 to-pink-600 text-white px-4 py-2 rounded-md">
